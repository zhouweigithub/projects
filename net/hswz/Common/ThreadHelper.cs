...
namespace Hswz.Common
{
    ...
    public class ThreadHelper
    {
        ...
        public static void Threading<T>(List<T> source, Int32 maxThreadCount, Int32 waitSeconds, Action<T> action)
        {
            ...
            Int32 threadCount = source.Count < maxThreadCount ? source.Count : maxThreadCount;    //需要分配的最大线程数量
            ...
            {   //循环创建线程
                System.Threading.Tasks.Task.Factory.StartNew(() =>
                {
                    ...
                    {
                        Boolean isGetDataSuccess = false;        //是否从队列中取到了资源
                        T item = default(T);
                        lock (lockobj)
                        {
