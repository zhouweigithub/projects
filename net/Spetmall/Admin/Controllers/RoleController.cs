...
namespace Spetmall.Admin.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        //单例
        private static readonly DAL.RoleDAL bllrole = DAL.RoleDAL.GetInstance();
        ...
        {
            Model.RoleInfoListModel model = RoleDAL.GetInstance().GetListRoles();
            return View(model);
        }
        ...
        public ActionResult Add(Model.RoleModel model)
        {
            ...
            try
            {
                ...
                if (model.Id == 0)
                {
                    iReturn = bllrole.Add<Model.RoleModel>(model);
                }
                else
                {
                    iReturn = bllrole.UpdateByKey<Model.RoleModel>(model, model.Id);
                }
                if (iReturn > 0)
                {
                    result = true;
        ...
        public string IsExsitRole(string Id, string RolesName)
        {
            ...
            return bllrole.IsExsitRole(model).ToString();
        }
