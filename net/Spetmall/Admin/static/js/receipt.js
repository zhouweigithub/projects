...
var ReceiptCommon = function(){
	...
	this.setReceipt = function(_config){
		...
		try{
			...
			that.Ele.shoppingcartTotalprice.html( goods_price( _config.activitytotalprice/100 )  );
			that.Ele['youhui_total_price'].html(  goods_price( (_config.originaltotalprice - _config.activitytotalprice)/100 ) );
	...
	this.selectUser = function(_config){
		...
		this.Ele['huiyuaninfo'].find('.uname').text(_config['username']);
		this.Ele['huiyuaninfo'].find('.uphone').text(_config['phone']);
		this.Ele['huiyuaninfo'].find('.uintegral').text(_config['integral']);
		this.Ele['huiyuaninfo'].find('.utotal_balance').text(goods_price(_config['total_balance']/100));
		this.Ele['huiyuaninfo'].find('.uaddtime').text(getLocalTime(_config['add_time']));
	...
	this.MembershipDiscount = function(_discount_type){
		...
		_externalframeEle.attr('src',replace_param(_externalframeEle.attr('src'),'discount_type',_discount_type));
	...
	this.selectUser2 = function(_config){
		...
		_notlogin.hide();
		_receiptmemberinfos.find('.r_username').text(_config['username']);
		_receiptmemberinfos.find('.r_total_balance').text(goods_price(_config['total_balance']/100));
		_receiptmemberinfos.find('.r_integral').text(_config['integral']);
		_receiptmemberinfos.find('.r_discount').text(_config['discount']);
		$("input[name=memberuid]").val(_config['id']);
		if(that.OpenDialog) that.OpenDialog.close();
		...
		$.post('/Receipt/receiptShoppingCart.html?_t='+new Date().getTime(),{shoppinggoods:JSON.stringify(that.ShoppingGoods),uid:_config['id']},function(_RESULT){
		    ...
		    that.Ele.shoppingcartTotalprice.html(goods_price(_RESULT['activitytotalprice']/100));
		},'json');
	}
	...
	this.setReceiptdata = function(_config){
		 ...
		 this.Ele['confiem_preferential_amount'].html( goods_price((_config['originaltotalprice'] - _config['activitytotalprice'])/100 ));
		 this.Ele['confiem_total'].val( goods_price(_config['activitytotalprice']/100) );
	...
	function create_item(_data,_reload){
		...
		_data['unitprice'] = (_data['myprice']/100).toFixed(2);
	...
	function create_goods(_data){
		_data['my_price'] = goods_price(_data['myprice']/100);
		
		if(_data['storage']<=_data['alarm']){
	...
	function updateTotalPrice(_dom,_operationtype){
		...
		CalculatingCommodityPrices(that.CartData,function(_result){
			    ...
			    _dom.attr("data-activityprice",_result.goods_activity_total_price);
		     	that.Ele.shoppingcartTotalprice.html(( _result.activitytotalprice/100 ) . toFixed(2) );
				that.Ele['youhui_total_price'].html(  goods_price( (_result['originaltotalprice'] - _result['activitytotalprice'])/100 ) );
	...
	function paginate(_callback){
	   ...
	   $.get('/Receipt/paginate.html',that.pagewhere,function(_RESULT){
	...
	function CashRegisterCallback(_RESULT){
	    art.dialog({icon:'succeed',id:'submitloading',lock: true,width:200,title:"消息提示",content:_RESULT['msg'],button:[
		{
			name: '继续收银',
			callback: function () {
	...
	function CalculatingCommodityPrices(_CartData,_callback){
		...
		$.post("/Receipt/calculatingCommodityPrices.html?isAjax=1&_t="+ new Date().getTime(),{
			uid:strToint($("input[name=memberuid]").val()),
	...
	var template = function(){
			...
			'        <span class="num" data-number="{#cart_number}"  data-goods-id="{#id}"  data-price="{#myprice}">{#cart_number}</span>',
