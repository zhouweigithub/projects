...
var ArtSpecData = function(){
	...
	this.setGuizhe = function(_data){
		...
		for(var i in _data){
			...
			_data[i]['min_price'] = _data[i]['min_price']/100 ;
			_data[i]['reduce_price'] = _data[i]['reduce_price']/100 ;
	...
	function _Dialog(_CALLBACK){
		windowdialog =  art.dialog.open("/Activity/getSpecData.html",{
				  title:'请选择商品',
				  width:'90%',
				  height:'90%',
				  fixed:true,
	...
	function template(){
		return ['<div class="goodsguizhe_attrs">',
					'               		<div class="goodsguizhe_attr">单笔订单满：<input name="content[min_price][{#specconfigid}]" type="text" class="txt" value="{#min_price}" /> , 立减现金<input name="content[reduce_price][{#specconfigid}]" type="text" class="txt" value="{#reduce_price}" />  , 送礼物 <input name="formsubmits" id="specbutton" type="button" data-index="{#specconfigid}" class="btn dssc-btn" value="选择商品"/> <a href="javascript:;" class="specdel">删除</a></div>',
	...
	function Discounttemplate(){
		return ['<div class="goodsguizhe_attrs">',
					'               		<div class="goodsguizhe_attr">订单满：<input name="content[min][{#specconfigid}]" type="text" class="txt" value="{#min}" />{#company},折扣<input name="content[discount][{#specconfigid}]" type="text" class="txt" value="{#discount}" /><a href="javascript:;" class="specdel">删除</a></div>',
					'                    <div class="goodsguizhe_attr_list">',
					'                    </div>',
					'</div>'].join("\n");
	}
